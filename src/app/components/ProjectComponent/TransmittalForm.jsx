...
const TransmittalForm = () => {
  ...
  const sanitizeProjectForFile = (name) =>
    String(name || "")
  ...
  useEffect(() => {
    ...
    const setIfEmpty = (ref, val) => {
      ...
      if (ref.current.value && String(ref.current.value).trim().length > 0)
        return;
      if (val == null) return;
      try {
        ref.current.value = String(val);
      } catch {}
    };
    ...
    const resolveAndFill = async () => {
      try {
        ...
        const match = list.find((p) => {
          const idMatch =
            projectNo &&
            !isNaN(Number(projectNo)) &&
            Number(p.id) === Number(projectNo);
          const noMatch =
            projectNo && String(p.projectNo) === String(projectNo);
          const nameMatch =
            projectName &&
            (String(p.name) === String(projectName) ||
              String(p.projectName) === String(projectName));
        ...
        const clientProjNo =
          match?.ClientprojectNo ||
          match?.clientProjectNo ||
          match?.client_project_no ||
          match?.clientProjNo ||
          match?.client_proj_no ||
          match?.client?.clientJobNo ||
          match?.fabricatorJobNo ||
          match?.jobNo ||
          match?.projectJobNo ||
          "";
        ...
        const pmName =
          ...
          match?.clientPm?.name ||
          match.coordinator ||
          match.fabricatorCoordinator ||
          match.client?.coordinator ||
          match.client?.contactPerson ||
          "";
        setIfEmpty(coordinatorRef, pmName);
        // 3) SOL JOB NO
        setIfEmpty(
          solJobNoRef,
          match.solJobNo || match.solJob || match.projectNo || ""
        );
        // 4) FABRICATOR NAME -> Prefer Client.name; fallback to embedded project.client
        setIfEmpty(
          fabricatorNameRef,
          match.client?.name || match.fabricatorName || match.clientName || ""
        );
        // 5) SOL Team Leader
        setIfEmpty(
          teamLeaderRef,
          match.solTL?.name || match.teamLeader || match.teamLead || ""
        );
        // Last 4 fields (Transmittal Name, Submittal Name, Zip Name, Complete Name) are intentionally left manual.
      } catch (e) {
        console.warn(
          "Failed to prefill project header fields",
          e?.message || e
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const loadClientName = async () => {
      ...
      try {
        ...
        if (
          match?.name &&
          fabricatorNameRef?.current &&
          (!fabricatorNameRef.current.value ||
            fabricatorNameRef.current.value.trim().length === 0)
        ) {
          try {
            fabricatorNameRef.current.value = String(match.name);
          } catch {}
        }
        if (
          match?.clientJobNo &&
          jobNoRef?.current &&
          (!jobNoRef.current.value ||
            jobNoRef.current.value.trim().length === 0)
        ) {
          try {
            jobNoRef.current.value = String(match.clientJobNo);
          } catch {}
        }
      } catch {}
    };
  ...
  const tableDrawings = selectedDrawings.map((d, i) => {
    ...
    return {
      ...
      rev: rev,
      dateSentForApproval: isAlphaRev
        ? today
        : d.approvalDate || d.dateSentForApproval || "",
      dateReceivedBFA: d.bfaDate || "",
      dateSentForFab: isNumericRev
        ? today
        : d.fabDate || d.dateSentForFab || "",
  ...
  const createCompleteLogBlob = async (projectIdNum, pkgIdResolved) => {
    ...
    const res = await fetch(`/api/project-drawings?${qp.toString()}`, {
      cache: "no-store",
    });
    if (!res.ok) {
    ...
    ws.addRow([
      ...
      projectName || "",
      "PACKAGE",
      selectedPackageName || (pkgIdResolved ? `#${pkgIdResolved}` : "All"),
    ]);
    ...
    const normalizeCreatedAt = (row) => {
      ...
      let v =
        row?.createdAt ?? row?.metadata?.createdAt ?? row?.meta?.createdAt;
      try {
        if (!v) return "";
    ...
    rows.forEach((r, idx) => {
      ...
      const status =
        rawStatus && rawStatus.toUpperCase() === "SUSPENDED"
          ? "VOID"
          : rawStatus;
      let fileNames = "";
      try {
        if (r.fileName) fileNames = String(r.fileName);
        else if (r.meta && Array.isArray(r.meta.fileNames))
          fileNames = r.meta.fileNames.join(", ");
        else if (r.metadata && Array.isArray(r.metadata.fileNames))
          fileNames = r.metadata.fileNames.join(", ");
      } catch {}
    ...
    ws.autoFilter = {
      ...
      to: { row: headerRowIndex + rows.length, column: 8 },
    };
  ...
  const handleDownload = async () => {
    ...
    tableDrawings.forEach((d) => {
      const hasDateSentForApproval =
        d.dateSentForApproval && d.dateSentForApproval.trim() !== "";
      const hasDateSentForFab =
        d.dateSentForFab && d.dateSentForFab.trim() !== "";
    ...
    y += 80;
    const todayDate = new Date();
    const formattedDate = `${String(todayDate.getMonth() + 1).padStart(
      2,
      "0"
    )}-${String(todayDate.getDate()).padStart(
      2,
      "0"
    ...
    const pdfTableHeaders = [
      ["REV. REMARK", "SHEET TITLE", "SHEET NAME", "SHEET QTY"],
    ];
    ...
    try {
      ...
      let projectIdNum =
        selectedProjectId != null && /^\d+$/.test(String(selectedProjectId))
      ...
      const normalizeCategory = (cat) => {
        ...
        if (
          ...
          return "G";
        if (c.includes("PART") || c.includes("COMPONENT") || c === "P")
          return "W";
        return c[0] || "";
      };
      ...
      const resolvePackageId = async (projId) => {
        ...
        if (
          ...
          /^\d+$/.test(String(selectedPackageId))
        ) {
        ...
        try {
          const resp = await fetch(
            `/api/packages?projectId=${Number(projId)}`,
            { cache: "no-store" }
          );
          if (!resp.ok) return undefined;
          const list = await resp.json();
          const needle = String(selectedPackageName || "")
            .trim()
            .toLowerCase();
          ...
          const match = (list || []).find((p) => {
            const nm = String(p?.name || "")
              .trim()
              .toLowerCase();
            const pn = String(p?.packageNumber || "")
              .trim()
              .toLowerCase();
    ...
    const addToZipByExtension = (filesArray) => {
      ...
      filesArray.forEach((file) => {
        ...
        const actualFile = file?.file || file;
        const ext =
          actualFile.name.split(".").pop()?.toUpperCase() || "UNKNOWN";
    ...
    const transBaseName =
      (transmittalName && transmittalName.trim()) ||
      makeLogBase("DrawingTransmittalLog");
    ...
    if (completeLogEnabled) {
      try {
        ...
        if (projectIdNum) {
          try {
            const resp = await fetch(
              `/api/packages?projectId=${Number(projectIdNum)}`,
              { cache: "no-store" }
            );
            if (resp.ok) {
              const list = await resp.json();
        ...
        if (projectIdNum) {
          const completeBlob = await createCompleteLogBlob(
            projectIdNum,
            pkgIdResolved
          );
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!clientId || !Number.isFinite(Number(clientId))) {
        throw new Error(
          "Please select a client in the PublishDrawing page before publishing."
        );
      }
      ...
      const pkgDisplay =
        selectedPackageName ||
        (selectedPackageId != null ? `#${selectedPackageId}` : "None");
        ...
        try {
          ...
          try {
            ...
            if (cres.ok) {
              ...
              const c = (clist || []).find(
                (x) => String(x.id) === String(selectedClientId)
              );
          ...
          const resp = await fetch("/api/send-email", {
            ...
            body: JSON.stringify({
              ...
              storagePath: (
                res?.record?.storagePath ||
                res?.record?.storage_path ||
                res?.objectPath ||
                ""
              ).replace(/^gs:\/\/[^/]+\//, ""),
            }),
          ...
          if (resp.ok && respJson?.sent === true) {
            ...
            setPublishResult((prev) => ({
              ...(prev || {}),
              email: { sent: true, to: count },
            }));
            alert(`Email sent successfully to ${count} recipient(s).`);
          } else {
            const errMsg =
              respJson?.error || resp.statusText || "Unknown error";
            setPublishResult((prev) => ({
              ...(prev || {}),
      ...
      if (e?.message?.includes("NetworkError")) {
        ...
        errorMessage =
          "Failed to connect to server. Please ensure the development server is running.";
      }
    ...
    <div className="p-6 text-gray-800 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div>
          ...
          <input
            ref={coordinatorRef}
            className="border w-full px-2 py-1 rounded font-semibold"
          />
        </div>
        <div>
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm mt-4">
        <label className="flex items-center gap-1">
          <input
            ...
            onChange={(e) =>
              e.target.checked
                ? generateLogName("transmittal")
                : clearLogName("transmittal")
            }
          />{" "}
          Transmittal Log
        </label>
        <label className="flex items-center gap-1">
          <input
            ...
            onChange={(e) =>
              e.target.checked
                ? generateLogName("submittal")
                : clearLogName("submittal")
            }
          />{" "}
          Submittal Log
        </label>
        <label className="flex items-center gap-1">
          <input
            type="checkbox"
            onChange={(e) => setCompleteLogEnabled(e.target.checked)}
          />{" "}
          Complete Log
        </label>
      </div>
      ...
      <div className="mt-4 flex gap-2 flex-wrap">
        ...
        <button className="bg-teal-800 text-white px-4 py-2 rounded text-sm hover:bg-teal-900">
          Save
        </button>
        <button
                  ...
                  <div className="p-3 text-sm">
                    <div className="grid grid-cols-2 gap-y-2">
                      ...
                      <div
                        ...
                        title={
                          publishResult?.summary?.name ||
                          publishResult?.data?.record?.fileName
                        }
                      >
                        {publishResult?.summary?.name ||
                          publishResult?.data?.record?.fileName ||
                      ...
                      <div className="font-medium">
                        {publishResult?.summary?.clientId ?? "-"}
                      </div>
                      <div className="text-gray-500">Project ID</div>
                      <div className="font-medium">
                        {publishResult?.summary?.projectId ?? "-"}
                      </div>
                      <div className="text-gray-500">Upload time</div>
                      <div className="font-medium">
                        {publishResult?.summary?.uploadTime
                          ? new Date(
                              publishResult.summary.uploadTime
                            ).toLocaleString()
                          : "-"}
                  ...
                  <div className="mt-1 text-sm">
                    {publishResult.email.sent ? (
                      <p className="text-green-700">
                        Email: Sent to {publishResult.email.to || 0}{" "}
                        recipient(s).
                      </p>
                    ) : (
                      <p className="text-red-700">
                        Email: Failed
                        {publishResult.email.error
                          ? ` — ${publishResult.email.error}`
                          : ""}
                      </p>
                    )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white border rounded-md w-[600px] p-4 shadow-lg relative">
            ...
            <h2 className="font-bold mb-2">
              Attachments for Drawing {modalDrawingNo}
            </h2>
