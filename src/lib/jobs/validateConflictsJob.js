export async function handleValidateConflicts(job, prisma) {
  ...
  const drawingKeys = Array.isArray(payload.drawingKeys)
    ? payload.drawingKeys
    : [];
      ...
      const { normDr, normCat } = (() => {
        ...
        const normalizeCategory = (c) => {
          const cc = String(c || "")
            ...
            .toUpperCase();
          if (!cc) return "";
          if (cc === "A" || cc === "G" || cc === "W") return cc;
          if (cc.startsWith("SHOP") || cc === "S") return "A";
          if (
            cc.startsWith("ERECTION") ||
            cc === "E" ||
            cc === "GA" ||
            cc.includes("GENERAL")
          )
            return "G";
          if (cc.includes("PART") || cc.includes("COMPONENT") || cc === "P")
            return "W";
          return cc[0] || "";
        };
        // additionally strip any suffix after first hyphen (e.g., 'E101-RA' -> 'E101')
        const nd = stripFrTrSuffix(
          normalizeToken(String(dr).split("-")[0] || dr)
        );
        const nc = normalizeCategory(cat);
        return { normDr: nd, normCat: nc };
      })();
      const key = `${normDr}::${normCat}`;
      const revVal = String(row.revision || row.rev || "").trim();
      const ts = new Date(
        row.updatedAt || row.updated_at || row.lastAttachedAt || 0
      ).getTime();
      const prev = map[key];
      if (!prev || ts > prev._ts) {
        ...
        if (row.meta) {
          try {
            ...
            if (obj && Array.isArray(obj.fileNames)) fns.push(...obj.fileNames);
          } catch {}
        }
        if (row.fileName && typeof row.fileName === "string")
          fns.push(...row.fileName.split(/\s*,\s*/).filter(Boolean));
  ...
  const normKeys = drawingKeys.map((k) =>
    (String(k || "").split("-")[0] || String(k || "")).trim()
  );
  const placeholders = normKeys.map((_, i) => `$${i + 1}`).join(", ");
  const sql = `SELECT "drgNo", "category", revision, "fileName" FROM "ProjectDrawing" WHERE "projectId" = $${
    normKeys.length + 1
  } AND superseded_by IS NULL AND "drgNo" IN (${placeholders})`;
  ...
  const map = {};
  for (const r of rows) {
    const key = String(r.drgNo || "").trim();
    map[key] = { rev: r.revision || null, fileName: r.fileName || null };
  }
  const conflicts = drawingKeys.map((k) => {
    const nk = (String(k || "").split("-")[0] || String(k || "")).trim();
    return { drgNo: k, prev: map[nk] || null };
  });
  return { conflicts };
}
