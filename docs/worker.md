...
- **Dispatch**: Based on `job.type`, it calls one of the handlers:
...
- **On error**: worker logs error, increments attempts, and either re-queues (`queued`) or marks `failed` when `attempts >= maxAttempts` (default 5). The error message is saved on the job row.
...
- Input (job.payload):
  ...
  - other optional: `projectId`, `clientId`, `originalName`, `uploaderId`
- Behavior: decodes base64, reads first sheet using `xlsx`, locates header row (searches for a header containing `dr no`), extracts rows as arrays and maps columns to fields (`drgNo`, `description/item`, `rev`, `rev remarks`, `mod by`, `dr by`, `ch by`, `category`) and returns normalized rows as `result.rows`.
- Output (job.result): `{ rows: [ { id, slno, drgNo, item, rev, modeler, detailer, checker, status, category, view, attachedPdfs, conflict, attachConflict } ] }`
- Errors: throws if `fileBase64` missing or parsing fails; worker will mark job failed or requeue per attempts.
...
- Behavior:
  - If `fetchAll` true: queries `ProjectDrawing` rows for the project (and package if provided), normalizes drawing tokens and categories and builds a map keyed both by `normDr::normCat` and `normDr` and also `file::<lowercased filename>` to previous revision values. Returns `{ prevRevMap, rows }`.
  - Otherwise, if `drawingKeys` provided: queries matching `ProjectDrawing` rows and returns `{ conflicts: [ { drgNo, prev } ] }` where `prev` contains revision/fileName.
...
- Input (job.payload): expected shape:
  - `clientId`, `projectId`, `packageId` (optional)
  - `drawings`: array of entries like `{ drawingNumber, category, revision, fileNames, issueDate }`
- Behavior:
  ...
  - For each entry it: finds an existing active `ProjectDrawing` (matching projectId, packageId (if provided), drgNo and `superseded_by IS NULL`) FOR UPDATE, marks the previous row as superseded (updates `superseded_by` to -1 temporarily then later points to new id), inserts a new row with `fileName` and `meta` (JSON with fileNames and issueDate) and then updates the previous row `superseded_by` to the new id.
- Output: `{ created, superseded, total }` indicating counts.
- Notes: The function contains raw SQL (`$queryRawUnsafe`) and manual parameter binding to implement complex upsert logic and locking. It expects `projectId` to be present on entries.
...
- Behavior:
  - Uses `getGCSStorage()` (GCS client) to read files from the bucket, composes a ZIP using `archiver`, writes the ZIP to `gs://<bucket>/<destPath>` by creating a write stream to the destination file.
...
- Error handling: If files fail to add to archive, there are warnings per-file but the job proceeds; fatal errors reject and cause the job to fail.
...
- Publish flow (client-side `TransmittalForm.jsx`):
  ...
  2. Enqueue `publish-job` by POST /api/jobs/enqueue with payload `{ clientId, projectId, packageId, drawings: entriesToUpsert }`. Worker will upsert to `ProjectDrawing` table.
  ...
  4. Enqueue `generate-zip` with `objectPaths: uploadedObjectPaths` and optional `zipName`. Worker will read objects server-side and produce a central ZIP plus a `DocumentLog` record.

- Excel parsing flow (client-side `PublishDrawing.jsx`):

  1. When user selects an Excel file, the client encodes it to base64 and enqueues `parse-excel` payload `{ fileBase64, projectId, clientId, originalName }`.
  2. Client polls `/api/jobs/:id` until `status === 'succeeded'` and then reads `job.result.rows` to populate the drawings table.
...
- `generate-zip` expects `objectPaths` that can be `gs://...` references or object paths relative to `GCS_BUCKET` env var. The job determines the bucket by `GCS_BUCKET` env var or by parsing first `gs://` path.

**Concurrency and Safety**

- Atomic claiming: worker uses `findFirst` and `updateMany` with where { id, status: 'queued' } to avoid double-claim. This is a lightweight optimistic claim.
...
- Job `attempts` increments on claim and on failure. Worker enforces `maxAttempts` (default 5) to stop retrying and mark job as `failed`.
...
- Enqueue parse-excel (client):
  ...
  Body: { "type": "parse-excel", "payload": { "fileBase64": "<base64>", "projectId": 42, "clientId": 3, "originalName": "drgs.xlsx" } }

- Enqueue publish-job (client):

  POST /api/jobs/enqueue
  Body: { "type": "publish-job", "payload": { "clientId": 3, "projectId": 42, "packageId": 7, "drawings": [ { "drawingNumber": "A-101", "category": "A", "revision": "A", "fileNames": ["A-101-A.pdf"], "issueDate": "2025-11-20" } ] } }

- Enqueue generate-zip (client):

  POST /api/jobs/enqueue
  Body: { "type": "generate-zip", "payload": { "objectPaths": ["gs://my-bucket/uploads/A-101-A.pdf", "gs://my-bucket/uploads/A-102-A.pdf"], "zipName": "transmittal-42.zip", "projectId": 42, "clientId": 3 } }
